...
truth configsystem::Load () {
  ...
  for (SaveFile.ReadWord(Word, false); !SaveFile.Eof(); SaveFile.ReadWord(Word, false)) {
    ...
    for (int c = 0; c < Options; ++c) if (Word == Option[c]->Name) Option[c]->LoadValue(SaveFile);
  }
  return true;
}


void configsystem::Show (void (*BackGroundDrawer)(), void (*ListAttributeInitializer)(felist&), truth SlaveScreen) {
  int Chosen;
  ...
  for (;;) {
    ...
    if (SlaveScreen && ListAttributeInitializer) ListAttributeInitializer(List);
    List.SetFlags(SELECTABLE|(SlaveScreen ? DRAW_BACKGROUND_AFTERWARDS : 0) | (!SlaveScreen && !TruthChange ? FADE : 0));
    Chosen = List.Draw();
    festring String;
    if (Chosen < Options) TruthChange = Option[Chosen]->ActivateChangeInterface();
    else {
      Save();
      return;
    }
...
void configsystem::NormalStringDisplayer (const stringoption *O, festring &Entry) {
  if (!O->Value.IsEmpty()) Entry << O->Value; else Entry << '-';
}
...
truth configsystem::NormalStringChangeInterface (stringoption *O) {
  festring String;
  if (iosystem::StringQuestion(String, CONST_S("Set new ")+O->Description+':', v2(30, 30), WHITE, 0, 80, true, true) == NORMAL_EXIT) O->ChangeValue(String);
  return false;
}


truth configsystem::NormalNumberChangeInterface (numberoption* O) {
  O->ChangeValue(iosystem::NumberQuestion(CONST_S("Set new ")+O->Description+':', v2(30, 30), WHITE, true));
  return false;
}
