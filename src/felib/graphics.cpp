...
CursorState::CursorState (int ax, int ay, int avis, int newx, int newy, truth newvis) { x = ax; y = ay; vis = avis; graphics::curx = newx; graphics::cury = newy; if (newvis) graphics::showCursor(); else graphics::hideCursor(); }
CursorState::CursorState (int ax, int ay, int avis, int newx, int newy) { x = ax; y = ay; vis = avis; graphics::curx = newx; graphics::cury = newy; }
CursorState::CursorState (int ax, int ay, int avis, truth newvis) { x = ax; y = ay; vis = avis; if (newvis) graphics::showCursor(); else graphics::hideCursor(); }
CursorState::~CursorState () { if (vis != -666) { graphics::curx = x; graphics::cury = y; graphics::curvisible = vis; } }
...
bool graphics::isCursorVisible () { return (curvisible > 0); }
void graphics::showCursor () { ++curvisible; }
void graphics::hideCursor () { --curvisible; }

CursorState graphics::getCursorState () { return CursorState(curx, cury, curvisible); }
CursorState graphics::getCursorState (int newx, int newy, truth newvis) { return CursorState(curx, cury, curvisible, newx, newy, newvis); }
CursorState graphics::getCursorState (int newx, int newy) { return CursorState(curx, cury, curvisible, newx, newy); }
CursorState graphics::getCursorState (truth newvis) { return CursorState(curx, cury, curvisible, newvis); }
...
void graphics::BlitDBToScreen () {
  ...
  if (curvisible > 0 && curx >= -7 && cury >= 0 && curx+8 <= Res.X && cury < Res.Y) {
