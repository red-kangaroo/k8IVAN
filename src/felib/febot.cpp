...
febot::wordchain::wordchain (const febot::wordchain *WordChain, cfestring& NewWord) :
  String(++WordChain->String.begin(), WordChain->String.end())
{
...
void febot::Reply (festring &String) const {
  String.Empty();
  if (GetControlWordChain()->Link.empty()) return;
