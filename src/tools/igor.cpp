...
int Main (int, char **) {
  ...
  List.SetWidth(200);
  while ((Selected = List.Draw()) & FELIST_ERROR_BIT) if(Selected == ESCAPED) return 0;
  switch (Selected) {
  ...
  for (;;) {
    ...
    if (k >= 0x31 && k <= 0x34) Selected = k-0x31;
    else if (k == '+') CBitmap->AlterGradient(Cursor, TILE_V2, Selected, 1, false);
    else if (k == '-') CBitmap->AlterGradient(Cursor, TILE_V2, Selected, -1, false);
    else if (k == '>') CBitmap->AlterGradient(Cursor, TILE_V2, Selected, 1, true);
    else if (k == '<') CBitmap->AlterGradient(Cursor, TILE_V2, Selected, -1, true);
    else if (k == KEY_UP) CBitmap->Roll(Cursor, TILE_V2, v2(0, -1), TempBuffer);
    else if (k == KEY_DOWN) CBitmap->Roll(Cursor, TILE_V2, v2(0, 1), TempBuffer);
    else if (k == KEY_RIGHT) CBitmap->Roll(Cursor, TILE_V2, v2(1, 0), TempBuffer);
    else if (k == KEY_LEFT) CBitmap->Roll(Cursor, TILE_V2, v2(-1, 0), TempBuffer);
    else if (k == '=') {
    ...
    for (c = 0; c < (int)DrawQueue.size(); ++c) {
      B2.Src = DrawQueue[c];
