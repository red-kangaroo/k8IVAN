...
cchar *god::GetPersonalPronoun () const { return (GetSex() == MALE ? "He" : "She"); }
cchar *god::GetObjectPronoun () const { return (GetSex() == MALE ? "Him" : "Her"); }
...
void god::Pray () {
  ...
  if (!Timer) {
    if (Relation >= -RAND_N(500)) {
      ...
      if (!TryToAttachBodyPart(PLAYER) && !TryToHardenBodyPart(PLAYER)) PrayGoodEffect();
      ...
      if (Relation > 250 && !(RAND()%20)) {
        ...
        if (!Category) Category = ANY_CATEGORY;
        ...
        if (Gift) {
          ...
          if (Gift->IsBroken()) Gift = Gift->Fix();
               if (Relation == 1000) Gift->EditEnchantment(3);
          else if (Relation >= 500) Gift->EditEnchantment(2);
          else Gift->EditEnchantment(1);
  ...
  if (LastPray > -1) {
    ...
    if (Day >= 7) {
      Desc << "over a week ago.";
           ...
           if (Day > 1) Desc << Day << " days, ";
      else if (Day) Desc << "one day, ";
           if (Hour > 1) Desc << Hour << " hours, ";
      else if (Hour) Desc << "one hour, ";
      if (Day || Hour) Desc << "and " << Min << " minutes ago.";
      else Desc << Min << " minutes ago.";
    }
  } else {
...
void god::AdjustRelation (god *Competitor, int Multiplier, truth Good) {
  ...
  if (!Good && Adjustment > 0) Adjustment = -Adjustment;
...
void god::AdjustRelation (int Amount) {
  ...
  if (Relation < -1000) Relation = -1000;
  if (Relation > 1000) Relation = 1000;
}
...
void god::AdjustTimer (sLong Amount) {
  Timer += Amount;
  if (Timer < 0) Timer = 0;
  if (Timer > 1000000000) Timer = 1000000000;
}
...
character *god::CreateAngel (team *Team, int LifeBase) {
  ...
  for (int c = 0; c < 100; ++c) {
    ...
    if (game::GetCurrentArea()->IsValidPos(TryToCreate)) {
      ...
      if (LifeBase && GetType() != VALPURUS && (!(RAND()%5) || abs(Relation) == 1000)) {
        Angel = archangel::Spawn(GetType());
        Angel->SetLifeExpectancy(LifeBase/10, 0);
      } else {
        Angel = angel::Spawn(GetType());
        if (LifeBase) Angel->SetLifeExpectancy(LifeBase, 0);
      }
...
void god::PrintRelation () const {
  ...
  int rel = GetRelation();
       ...
       if (rel >= 1000) VerbalRelation = "greets you as a Champion of the Cause!";
  else if (rel > 750) VerbalRelation = "is extremely pleased.";
  else if (rel > 250) VerbalRelation = "is very pleased.";
  else if (rel > 50) VerbalRelation = "is pleased.";
  else if (rel > -50) VerbalRelation = "is content.";
  else if (rel > -250) VerbalRelation = "is angry.";
  else if (rel > -750) VerbalRelation = "is very angry.";
  else if (rel >= -1000) VerbalRelation = "is extremely angry.";
...
truth god::ReceiveOffer (item *Sacrifice) {
  if (Sacrifice->SpecialOfferEffect(GetType())) return true;
  ...
  if (OfferValue) {
    ...
    game::ApplyDivineAlignmentBonuses(this, (OfferValue >= 0 ? OfferValue : -OfferValue)/5, true);
...
struct materialsorter {
  ...
  truth operator () (cmaterial *M1, cmaterial *M2) const { return (M1->GetHardenModifier(Item) > M2->GetHardenModifier(Item)); }
  citem *Item;
};
...
truth god::TryToAttachBodyPart (character *Char) {
  ...
  if (!Char->HasAllBodyParts()) {
    ...
    if (BodyPart && LikesMaterial(BodyPart->GetMainMaterial()->GetDataBase(), Char)) {
      ...
      for (uInt c = 0; c < MaterialVector.size(); ++c) {
      ...
      for (uInt c = 0; c < MaterialVector.size(); ++c) delete MaterialVector[c];
    }
...
truth god::TryToHardenBodyPart (character *Char) {
  ...
  for (uInt c = 1; c < uInt(Char->GetBodyParts()); ++c) { // annoying :(
...
void god::ApplyDivineTick () {
  if (Timer > 0) --Timer;
  if (LastPray > -1 && LastPray < 336000) ++LastPray;
}
