  ...
  LocationName(LocationName)
{
  ...
  if (IsInside) Flags |= FLUID_INSIDE;
  Liquid->SetMotherEntity(this);
  Emitation = Liquid->GetEmitation();
  AddLiquid(Liquid->GetVolume());
}
...
void fluid::Save (outputfile &SaveFile) const {
  ...
  if (GearImage) {
    ...
    for (int c = 0; c < Images; ++c) GearImage[c].Save(SaveFile);
  } else {
    SaveFile.Put(false);
  }
}
...
void fluid::Draw (blitdata &BlitData) const {
  ...
  if (MotherItem && BlitData.CustomData & ALLOW_ANIMATE) Image.Animate(BlitData, SpecialFlags);
}
...
truth fluid::FadePictures () {
  ...
  if (GearImage) {
    int Images = Flags & HAS_BODY_ARMOR_PICTURES ? BODY_ARMOR_PARTS : 1;
    for (int c = 0; c < Images; ++c) GearImage[c].Fade();
  }
  return Change;
}
...
void fluid::AddFluidInfo (const fluid *Fluid, festring &String) {
  ...
  for (; Fluid; Fluid = Fluid->Next) {
    ...
    if (LiquidBlood) {
      if (Blood) OneBlood = false; else Blood = true;
    }
  }
  if (Index <= 3) {
...
/* Used only when loading fluids. Correcting RandMap here is somewhat a gum solution. */
...
void fluid::CheckGearPicture (v2 ShadowPos, int SpecialFlags, truth BodyArmor) {
  ...
  if (BodyArmor) {
    ...
    if (GearImage) {
      if (GearImage[Index].ShadowPos != ShadowPos) GearImage[Index].Clear(false);
      else return; // the picture already exists and is correct
    } else {
      GearImage = new imagedata[BODY_ARMOR_PARTS];
      for (int c = 0; c < BODY_ARMOR_PARTS; ++c) new (&GearImage[c]) imagedata(false);
    }
    ImagePtr = &GearImage[Index];
    Pixels = (Image.AlphaSum * BodyArmorPartPixels[Index] / HUMAN_BODY_ARMOR_PIXELS) >> 10;
  } else {
    if (GearImage) {
      if (GearImage->ShadowPos != ShadowPos) GearImage->Clear(false);
      else return; // the picture already exists and is correct
    } else {
...
void fluid::DrawGearPicture (blitdata &BlitData, int SpecialFlags) const {
  ...
  if (BlitData.CustomData & ALLOW_ANIMATE) GearImage->Animate(BlitData, SpecialFlags);
}
...
void fluid::DrawBodyArmorPicture (blitdata &BlitData, int SpecialFlags) const {
  ...
  if (BlitData.CustomData & ALLOW_ANIMATE) GearImage[Index].Animate(BlitData, 0);
}
...
fluid::imagedata::imagedata (truth Load) :
  Picture(0),
  DripTimer(0),
  AlphaSum(0),
  ShadowPos(ERROR_V2)
...
void fluid::imagedata::AddLiquidToPicture (const rawbitmap *Shadow, sLong Pixels, sLong AlphaSuggestion, col16 Color, pixelpredicate PixelPredicate) {
  ...
  if (Shadow) {
    for (int x = 1; x < 14; ++x) {
      for (int y = 1; y < 14; ++y) {
        if (ValidityMap[x] & (1 << y) && !(Shadow->*PixelPredicate)(ShadowPos + v2(x, y))) {
  ...
  for (sLong c = 0; c < Lumps; ++c) {
    ...
    for (int d = 0; d < 8; ++d) {
      if (Pixels > RAND()%RoomForPixels) {
        ...
        if (!Shadow || (!(Shadow->*PixelPredicate)(ShadowPos + Pos) && ValidityMap[Pos.X] & (1 << Pos.Y))) {
          --Pixels;
          ...
          if (!Pixels) break; // implies c + 1 == Lumps
        }
      }
      --RoomForPixels;
    }
...
void fluid::imagedata::Clear (truth InitRandMap) {
  ...
  Picture->FillAlpha(0);
  AlphaSum = 0;
  if (InitRandMap) Picture->InitRandMap();
}
...
truth fluid::UseImage () const {
  return !(Flags & FLUID_INSIDE) && (!MotherItem || MotherItem->ShowFluids());
}
...
void fluid::StepOnEffect (character *Stepper) {
  if (!Liquid->GetStickiness() || Stepper->IsImmuneToStickiness()) return;
