...
void loricatus::PrayGoodEffect () {
  if (!game::GetLoricatusHammer()) {
    if (GetRelation() == 1000) {
      ...
      /*
      thunderhammer *Weapon = thunderhammer::Spawn(HAMMER);
      Weapon->InitMaterials(MAKE_MATERIAL(ILLITHIUM), MAKE_MATERIAL(SAPPHIRE), true);
  ...
  //
  if (PLAYER->GetUsableArms()) ADD_MESSAGE("You feel a slight tingling in your hands.");
  else ADD_MESSAGE("You feel a slight tingle.");
}
