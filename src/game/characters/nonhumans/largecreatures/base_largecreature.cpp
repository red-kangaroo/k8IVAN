...
{
public:
  virtual void CalculateSquaresUnder () override { SquaresUnder = 4; }
  virtual int GetSquareIndex (v2) const override;
  virtual int GetNeighbourSquares () const override { return 12; }
  ...
  virtual truth CanMoveOn (const square *) const override;
  virtual void PutTo (v2) override;
  virtual void Remove () override;
  virtual truth CreateRoute () override;
  virtual truth CanTheoreticallyMoveOn (const lsquare *) const override;
  virtual int GetFlySymbolSquareIndex () const override { return 1; }
  virtual int GetSwimmingSymbolSquareIndex () const override { return 3; }
  virtual int GetUnconsciousSymbolSquareIndex () const override { return 2; }
  virtual truth PlaceIsIllegal (v2, v2) const override;
...
protected:
  ...
  virtual void LoadSquaresUnder () override;
};
...
truth largecreature::IsFreeForMe (square *Square) const {
  ...
  for (int c = 0; c < 4; ++c) {
    ...
    if (!Area->IsValidPos(SquarePos) ||
        (Area->GetSquare(SquarePos)->GetCharacter() && Area->GetSquare(SquarePos)->GetCharacter() != static_cast<ccharacter*>(this)))
    {
      return false;
    }
  }
...
truth largecreature::CanTheoreticallyMoveOn (const lsquare *LSquare) const {
  ...
  for (int c = 0; c < 4; ++c) {
    v2 SquarePos = Pos+game::GetLargeMoveVector(12+c);
    if (!Level->IsValidPos(SquarePos) || !(GetMoveType() & Level->GetLSquare(SquarePos)->GetTheoreticalWalkability())) return false;
  }
  return true;
}
...
truth largecreature::CanMoveOn (const lsquare *LSquare) const {
  ...
  for (int c = 0; c < 4; ++c) {
    v2 SquarePos = Pos+game::GetLargeMoveVector(12+c);
    if (!Level->IsValidPos(SquarePos) || !PartCanMoveOn(Level->GetLSquare(SquarePos))) return false;
  }
  return true;
}
...
truth largecreature::CanMoveOn (const square *Square) const {
  ...
  for (int c = 0; c < 4; ++c) {
    v2 SquarePos = Pos+game::GetLargeMoveVector(12+c);
    if (!Area->IsValidPos(SquarePos) || !(GetMoveType() & Area->GetSquare(SquarePos)->GetSquareWalkability())) return false;
  }
  return true;
}
...
truth largecreature::PlaceIsIllegal (v2 Pos, v2 Illegal) const {
  for (int c = 0; c < 4; ++c) {
    if (Pos+game::GetLargeMoveVector(12+c) == Illegal) return true;
  }
  return false;
}
...
truth largecreature::PartCanMoveOn (const lsquare *LSquare) const {
  ...
  if (GetMoveType()&Walkability) return true;
  if (DestroysWalls() && (Walkability&ETHEREAL)) {
    ...
    if (Terrain && Terrain->WillBeDestroyedBy(this)) {
      ...
      if (!Room || Room->IsOKToDestroyWalls(this)) return true;
    }
  }
  return false;
}
