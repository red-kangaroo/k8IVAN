...
#define CALL_EXPAND(x, y) do { \
  ExpandPossibleRoute(x, y, TargetX, TargetY, XMode); \
  if (FlagMap[TargetX][TargetY]&ON_POSSIBLE_ROUTE) return; \
} while(0)
...
void level::ExpandPossibleRoute (int OrigoX, int OrigoY, int TargetX, int TargetY, truth XMode) {
  ...
  if (XMode) {
    if (TargetX < OrigoX && CHECK(OrigoX-1, OrigoY)) CALL_EXPAND(OrigoX-1, OrigoY);
    if (TargetX > OrigoX && CHECK(OrigoX+1, OrigoY)) CALL_EXPAND(OrigoX+1, OrigoY);
    if (TargetY < OrigoY && CHECK(OrigoX, OrigoY-1)) CALL_EXPAND(OrigoX, OrigoY-1);
    if (TargetY > OrigoY && CHECK(OrigoX, OrigoY+1)) CALL_EXPAND(OrigoX, OrigoY+1);
    if (TargetX <= OrigoX && OrigoX < XSize-2 && CHECK(OrigoX+1, OrigoY)) CALL_EXPAND(OrigoX+1, OrigoY);
    if (TargetX >= OrigoX && OrigoX > 1 && CHECK(OrigoX-1, OrigoY)) CALL_EXPAND(OrigoX-1, OrigoY);
    if (TargetY <= OrigoY && OrigoY < YSize-2 && CHECK(OrigoX, OrigoY+1)) CALL_EXPAND(OrigoX, OrigoY+1);
    if (TargetY >= OrigoY && OrigoY > 1 && CHECK(OrigoX, OrigoY-1)) CALL_EXPAND(OrigoX, OrigoY-1);
  } else {
    if (TargetY < OrigoY && CHECK(OrigoX, OrigoY-1)) CALL_EXPAND(OrigoX, OrigoY-1);
    if (TargetY > OrigoY && CHECK(OrigoX, OrigoY+1)) CALL_EXPAND(OrigoX, OrigoY+1);
    if (TargetX < OrigoX && CHECK(OrigoX-1, OrigoY)) CALL_EXPAND(OrigoX-1, OrigoY);
...
#define CALL_EXPAND(x, y) do { \
  ...
  if (FlagMap[TargetX][TargetY]&STILL_ON_POSSIBLE_ROUTE) return; \
} while(0)
...
truth level::MakeRoom (const roomscript *RoomScript) {
  ...
  for (int x = Pos.X-1; x <= Pos.X+Size.X; ++x) {
    for (int y = Pos.Y-1; y <= Pos.Y+Size.Y; ++y) {
      if (FlagMap[x][y]&FORBIDDEN || FlagMap[x][y]&PREFERRED) return false;
    }
  }
  ...
  if (*RoomScript->GenerateTunnel() && !Door.empty()) {
    ...
    if (OKForDoor.empty()) ABORT("The Doors - You are strange.");
    ...
    if (Door && !(RAND()%5) && *RoomScript->AllowLockedDoors()) {
      if (*RoomScript->AllowBoobyTrappedDoors() && !(RAND()%5)) Door->CreateBoobyTrap();
      Door->Lock();
    }
  ...
  if (*RoomScript->GenerateDoor()) {
    ...
    if (OKForDoor.empty()) ABORT("The Doors - This thing has been broken.");
  ...
  if (ItemMap) {
    ...
    for (int x = 0; x < ItemMap->GetSize()->X; ++x) {
      game::BusyAnimation();
      for (int y = 0; y < ItemMap->GetSize()->Y; ++y) {
...
void level::Load (inputfile &SaveFile) {
  ...
  for (int x = 0; x < XSize; ++x) {
    for (int y = 0; y < YSize; ++y) {
      if (!Map[x][y]->IsInside()) Map[x][y]->AmbientLuminance = AmbientLuminance;
      NodeMap[x][y] = new node(x, y, Map[x][y]);
  ...
  for (int c1 = 0; c1 < HowMany; ++c1) {
    ...
    for (int c2 = 0; c2 < 30; ++c2) {
      ...
      if ((!Square->GetRoomIndex() || !Square->GetRoom()->DontGenerateMonsters()) &&
          (!ConsiderPlayer || (Pos-PLAYER->GetPos()).GetManhattanLength() > 6)) break;
    }
    if (Pos != ERROR_V2) {
      Char->PutTo(Pos);
      Char->SetGenerationDanger(Difficulty);
...
v2 level::GetRandomSquare (ccharacter* Char, int Flags, const rect* Borders) const {
  ...
  for (int c = 0;; ++c) {
    ...
    if ((RoomFlags == IN_ROOM && !LSquare->GetRoomIndex()) || (RoomFlags == NOT_IN_ROOM && LSquare->GetRoomIndex())) continue;
    return Pos;
  }
}
...
room *level::GetRoom (int I) const {
  if (!I) ABORT("Access to room zero denied!");
  return Room[I];
}
...
void level::Explosion (character *Terrorist, cfestring &DeathMsg, v2 Pos, int Strength, truth HurtNeutrals) {
  ...
  int Size = 6;
  //
  for (int c = 0; c < 6; ++c) if (Strength >= StrengthLimit[c]) { Size = c; break; }
  ...
  if (ExplosionQueue.size() == 1) {
    ...
    for (unsigned int c = 0; c < ExplosionQueue.size(); ++c) delete ExplosionQueue[c];
...
truth level::DrawExplosion (const explosion *Explosion) const {
  ...
  if (BPos.X >= RES.X || BPos.Y >= RES.Y) return false;
  if (BPos.X+SizeVect.X > RES.X) SizeVect.X = RES.X-BPos.X;
...
int level::TriggerExplosions (int MinIndex) {
  ...
  for (c = MinIndex; c < LastExplosion; ++c) {
    ...
    if (!GetSquare(ExplosionQueue[c]->Pos)->CanBeSeenByPlayer(true)) ++NotSeen;
  }
  if (NotSeen) {
  ...
  truth Drawn = false;
  for (c = MinIndex; c < LastExplosion; ++c) if (DrawExplosion(ExplosionQueue[c])) Drawn = true;
  if (Drawn) {
  ...
  for (c = MinIndex; c < LastExplosion; ++c) {
    ...
    if (GetLSquare(Explosion->Pos)->IsFlyable()) GetLSquare(Explosion->Pos)->AddSmoke(gas::Spawn(SMOKE, 1000));
  }
  for (c = MinIndex; c < LastExplosion; ++c) GetLSquare(ExplosionQueue[c]->Pos)->SetTemporaryEmitation(0);
...
void level::GasExplosion (gas* GasMaterial, lsquare* Square, character* Terrorist) {
  for (int d = 0; d < 9; ++d) {
    lsquare *Neighbour = Square->GetNeighbourLSquare(d);
    if (Neighbour) {
      if (Neighbour->IsFlyable()) Neighbour->AddSmoke(static_cast<gas*>(GasMaterial->SpawnMore(1000)));
      if (Terrorist) {
