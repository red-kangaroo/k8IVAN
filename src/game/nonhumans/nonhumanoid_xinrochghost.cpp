...
void xinrochghost::GetAICommand () {
  if (GetHomeRoom()) StandIdleAI(); else /*humanoid*/character::GetAICommand();
}
...
void xinrochghost::CreateCorpse (lsquare* Square) {
  ...
  if (!game::GetCurrentLevel()->IsOnGround()) {
    ...
    for (c = 0; c < Tunnels; ++c) game::GetCurrentLevel()->AttachPos(game::GetCurrentLevel()->GetRandomSquare(0, NOT_WALKABLE|ATTACHABLE));
    ...
    for (c = 0; c < ToGround; ++c) {
      for (int i = 0; i < 50; ++i) {
        ...
        if (Walkables > 6) {
          ...
          if (Char) {
            if (Char->CanBeSeenByPlayer()) ADD_MESSAGE("%s is hit by a brick of earth falling from the roof!", Char->CHAR_NAME(DEFINITE));
