...
void id::AddName (festring &Name, int Case) const {
  ...
  if (!(Case & STRIPPED) && ShowMaterial() && AddMaterialDescription(Name, Articled)) Articled = false;
  if (Case & PLURAL) Name << GetNamePlural(); else AddNameSingular(Name, Articled);
...
void id::AddName (festring &Name, int Case, int Amount) const {
  if (Amount == 1) {
    ...
    if ((Case & ARTICLE_BIT) && (GetArticleMode() == FORCE_THE || (!GetArticleMode() && !(Case & INDEFINE_BIT)))) Name << "the ";
    Name << Amount << ' ';
...
void id::AddPostFix (festring &String, int) const {
  if (GetPostFix().GetSize()) String << ' ' << GetPostFix();
}
