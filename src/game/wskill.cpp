...
void weaponskill::Load (inputfile &SaveFile) { SaveFile >> (int&)Level >> (int&)Hits >> (int&)HitCounter; }

truth weaponskill::AddHit (int AddHits) {
  if (!AddHits) return false;
  HitCounter = 0;
  if (Hits <= 5000000-AddHits) Hits += AddHits; else Hits = 5000000;
  int OldLevel = Level;
  while (Level < 20 && Hits >= GetLevelMap(Level + 1)) ++Level;
  return Level != OldLevel;
}


truth weaponskill::SubHit (int SubHits) {
  if (!SubHits) return false;
  if (Hits >= SubHits) Hits -= SubHits; else Hits = 0;
  int OldLevel = Level;
  while (Level && Hits < GetLevelMap(Level)) --Level;
  return Level != OldLevel;
}
...
truth weaponskill::Tick () {
  if (Hits && HitCounter++ >= GetUnuseTickMap(Level)) {
    ...
    if (SubHit(GetUnusePenaltyMap(Level))) return true;
  }
  return false;
}
...
truth sweaponskill::IsSkillOf (citem *Item) const {
  return (ID == Item->GetID() && Weight == Item->GetWeight() && Config == Item->GetConfig());
}
