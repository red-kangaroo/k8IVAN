...
void stack::Draw (ccharacter *Viewer, blitdata &BlitData, int RequiredSquarePosition) const {
  ...
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) {
    if (i->GetSquarePosition() == RequiredSquarePosition && (i->CanBeSeenBy(Viewer) || game::GetSeeWholeMapCheatMode())) {
...
void stack::AddItem (item *ToBeAdded, truth RunRoomEffects) {
  ...
  if (ToBeAdded->IsAnimated()) SquareUnder->IncStaticAnimatedEntities();
  if (!game::IsGenerating()) {
    if (RunRoomEffects && GetLSquareUnder()->GetRoom()) GetLSquareUnder()->GetRoom()->AddItemEffect(ToBeAdded);
...
/* Removes all items. LastClean should be true only if the stack is being deleted (the default is false) */
void stack::Clean (truth LastClean) {
  ...
  while (Slot) {
    ...
    if (!(Flags & HIDDEN) && Item->IsAnimated() && !LastClean) {
      ...
      if (Square) Square->DecStaticAnimatedEntities();
    }
    if (LastClean && Item->GetSquaresUnder() == 1) delete Item; else Item->SendToHell();
    stackslot *Rubbish = Slot;
    Slot = Slot->Next;
    delete Rubbish;
    if (!LastClean) SignalEmitationDecrease(Item->GetSquarePosition(), Item->GetEmitation());
  }
}
...
void stack::Load (inputfile &SaveFile) {
  ...
  for (int c = 0; c < SavedItems; ++c) {
    ...
    else Top = Top->Next = new stackslot(this, Top);
    SaveFile >> *Top;
    Volume += (*Top)->GetVolume();
    Weight += (*Top)->GetWeight();
    if ((*Top)->GetSquarePosition() == CENTER) Emitation = game::CombineConstLights(Emitation, (*Top)->GetEmitation());
  }
  Items += SavedItems;
}


v2 stack::GetPos () const { return GetSquareUnder()->GetPos(); }


/* Returns whether there are any items satisfying the sorter or any visible items if it is zero */
truth stack::SortedItems (ccharacter *Viewer, sorter SorterFunction) const {
  if (Items) {
    for (stackiterator i = GetBottom(); i.HasItem(); ++i) {
      if ((SorterFunction == 0 || ((*i)->*SorterFunction)(Viewer)) && ((Flags & HIDDEN) || i->CanBeSeenBy(Viewer))) return true;
    }
  }
  return false;
}
  ...
  if (KickDamage) {
    ...
    if (GetItems() && GetLSquareUnder()->IsFlyable()) {///&& SquarePosition == CENTER)
      ...
      if (Item2) {
        ...
        if (!(!Item2->Exists() || Item2->GetPos() != GetPos())) Item2->Fly(Kicker, Direction, KickDamage * 3);
      }
    }
...
void stack::FillItemVector (itemvector &ItemVector) const {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) ItemVector.push_back(*i);
}
...
item *stack::GetItem (int I) const {
  int c = 0;
  for (stackiterator i = GetBottom(); i.HasItem(); ++i, ++c) if (c == I) return *i;
  return 0;
}
...
int stack::SearchItem (item *ToBeSearched) const {
  int c = 0;
  for (stackiterator i = GetBottom(); i.HasItem(); ++i, ++c) if (*i == ToBeSearched) return c;
  return -1;
}
...
int stack::SearchChosen (itemvector &ReturnVector, ccharacter *Viewer, int Pos, int Chosen, int Flags,
...
int stack::GetSideItems (int RequiredSquarePosition) const {
  int VisibleItems = 0;
  for (stackiterator i = GetBottom(); i.HasItem(); ++i)
    if (i->GetSquarePosition() == RequiredSquarePosition) ++VisibleItems;
  return VisibleItems;
}
...
int stack::GetVisibleItems (ccharacter *Viewer) const {
  int VisibleItems = 0;
  for (stackiterator i = GetBottom(); i.HasItem(); ++i)
    if (i->GetSquarePosition() == CENTER && i->CanBeSeenBy(Viewer)) ++VisibleItems;
  lsquare *Square = GetLSquareUnder();
  for (int c = 0; c < 4; ++c) {
    stack *Stack = Square->GetStackOfAdjacentSquare(c);
    if (Stack) VisibleItems += Stack->GetVisibleSideItems(Viewer, 3-c);
  }
  return VisibleItems;
}
...
int stack::GetNativeVisibleItems (ccharacter *Viewer) const {
  ...
  int VisibleItems = 0;
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) if (i->CanBeSeenBy(Viewer)) ++VisibleItems;
  return VisibleItems;
}
...
int stack::GetVisibleSideItems (ccharacter *Viewer, int RequiredSquarePosition) const {
  int VisibleItems = 0;
  for (stackiterator i = GetBottom(); i.HasItem(); ++i)
    if (i->GetSquarePosition() == RequiredSquarePosition && i->CanBeSeenBy(Viewer)) ++VisibleItems;
  return VisibleItems;
}
...
item *stack::GetBottomVisibleItem (ccharacter *Viewer) const {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i)
    if ((Flags & HIDDEN) || i->CanBeSeenBy(Viewer)) return *i;
  return 0;
}
...
void stack::SignalEmitationIncrease (int ItemSquarePosition, col24 EmitationUpdate) {
  ...
  if (!(Flags & FREEZED) && game::CompareLights(EmitationUpdate, Emitation) > 0) {
    ...
    if (MotherEntity) {
      if (MotherEntity->AllowContentEmitation()) MotherEntity->SignalEmitationIncrease(EmitationUpdate);
    } else {
...
void stack::SignalEmitationDecrease (int ItemSquarePosition, col24 EmitationUpdate) {
  if (ItemSquarePosition < CENTER) {
    stack *Stack = GetLSquareUnder()->GetStackOfAdjacentSquare(ItemSquarePosition);
    if (Stack) Stack->SignalEmitationDecrease(CENTER, EmitationUpdate);
    return;
  }
  if (!(Flags & FREEZED) && Emitation && game::CompareLights(EmitationUpdate, Emitation) >= 0) {
    ...
    if (Backup != Emitation) {
      if (MotherEntity) {
        if (MotherEntity->AllowContentEmitation()) MotherEntity->SignalEmitationDecrease(EmitationUpdate);
      } else {
...
col24 stack::GetSideEmitation (int RequiredSquarePosition) {
  col24 Emitation = 0;
  for (stackiterator i = GetBottom(); i.HasItem(); ++i)
    if (i->GetSquarePosition() == RequiredSquarePosition) game::CombineLights(Emitation, i->GetEmitation());
  return Emitation;
}


truth stack::CanBeSeenBy (ccharacter *Viewer, int SquarePosition) const {
  if (MotherEntity) return MotherEntity->ContentsCanBeSeenBy(Viewer);
...
truth stack::IsDangerous (ccharacter *Stepper) const {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i)
    if (i->IsDangerous(Stepper) && i->CanBeSeenBy(Stepper)) return true;
  return false;
}
...
void stack::MoveItemsTo (stack *Stack) {
  while (Items) GetBottom()->MoveTo(Stack);
}


void stack::MoveItemsTo (slot *Slot) {
  while (Items) Slot->AddFriendItem(*GetBottom());
}


item *stack::GetBottomItem (ccharacter *Char, truth ForceIgnoreVisibility) const {
  if ((Flags & HIDDEN) || ForceIgnoreVisibility) return Bottom ? **Bottom : 0;
...
truth stack::TakeSomethingFrom (character *Opener, cfestring &ContainerName) {
  if (!GetItems()) {
...
void stack::SortAllItems (const sortdata &SortData) const {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) i->SortAllItems(SortData);
}


/* Search for traps and other secret items */
void stack::Search (ccharacter *Char, int Perception) {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) i->Search(Char, Perception);
}
...
truth stack::NeedDangerSymbol (ccharacter *Viewer) const {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i)
    if (i->NeedDangerSymbol() && i->CanBeSeenBy(Viewer)) return true;
  return false;
}
...
void stack::FinalProcessForBone () {
  ...
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) i->FinalProcessForBone();
}
...
truth stack::DetectMaterial (cmaterial *Material) const {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) if (i->DetectMaterial(Material)) return true;
  return false;
}


void stack::SetLifeExpectancy (int Base, int RandPlus) {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) i->SetLifeExpectancy(Base, RandPlus);
}
...
void stack::CalculateEnchantments () {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) i->CalculateEnchantment();
}
...
void stack::Haste () {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) i->Haste();
}


void stack::Slow () {
  for (stackiterator i = GetBottom(); i.HasItem(); ++i) i->Slow();
}
