...
void scrollofgolemcreation::FinishReading (character* Reader) {
  for (;;) {
    ...
    if (!Item.empty()) {
      ...
      truth SecPossible = Sec && Sec->GetVolume() && Sec->GetCategoryFlags() & IS_GOLEM_MATERIAL && !Sec->IsSameAs(Main);
      ...
      sLong Volume = (MainPossible ? (Sec && Sec->IsSameAs(Main) ? Main->GetVolume() + Sec->GetVolume() : Main->GetVolume()) : Sec->GetVolume());
      Golem->SetItemVolume(Volume);
      v2 Where = GetLevel()->GetNearestFreeSquare(Golem, Reader->GetPos());
      if (itemcontainer* Container = dynamic_cast<itemcontainer*>(Item[0])) Container->GetContained()->MoveItemsTo(Reader->GetStack());
