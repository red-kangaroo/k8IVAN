...
void necromancer::GetAICommand () {
  ...
  if (!NearestEnemy) {
    if (!RAND_N(3) && TryToRaiseZombie()) return;
  } else {
    if (!RAND_N(6) && TryToRaiseZombie()) return;
  }
...
truth necromancer::TryToRaiseZombie () {
  for (int c = 0; c < game::GetTeams(); ++c) {
    for(std::list<character*>::const_iterator i = game::GetTeam(c)->GetMember().begin(); i != game::GetTeam(c)->GetMember().end(); ++i) {
      if (!(*i)->IsEnabled() && (*i)->GetMotherEntity() && (*i)->GetMotherEntity()->Exists() &&
          (GetConfig() == MASTER_NECROMANCER || (*i)->GetMotherEntity()->GetSquareUnderEntity()->CanBeSeenBy(this)))
      {
