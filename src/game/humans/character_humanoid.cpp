...
humanoid::~humanoid () {
  for (std::list<sweaponskill*>::iterator i = SWeaponSkill.begin(); i != SWeaponSkill.end(); ++i) delete *i;
}
...
truth humanoid::Hit (character *Enemy, v2 HitPos, int Direction, int Flags) {
  ...
  int c, AttackStyles;
  for (c = 0, AttackStyles = 0; c < 8; ++c) if (GetAttackStyle()&(1<<c)) ++AttackStyles;
  ...
  if (CanUseEquipment()) {
    for (c = 0; c < Equipments; ++c) {
      if (GetBodyPartOfEquipment(c) && EquipmentIsAllowed(c)) {
        ...
        if (!AnimationDraw) DOUBLE_BUFFER->DrawRectangle(Pos+v2(-1, -1), Pos+TILE_V2, DARK_GRAY);
        item *Equipment = GetEquipment(c);
        if (Equipment && (!AnimationDraw || Equipment->IsAnimated())) {
          igraph::BlitBackGround(Pos, TILE_V2);
          B1.Dest = Pos;
          if (Equipment->AllowAlphaEverywhere()) B1.CustomData |= ALLOW_ALPHA;
          Equipment->Draw(B1);
...
double humanoid::GetTimeToKill (ccharacter *Enemy, truth UseMaxHP) const {
  ...
  if (IsUsingArms()) {
    ...
    if (RightArm || LeftArm) {
      if (RightArm) {
        ...
        if (Damage) Effectivity += 1/(Enemy->GetTimeToDie(this, int(Damage)+1, RightArm->GetToHitValue(), AttackIsBlockable(GetRightWielded() ? WEAPON_ATTACK : UNARMED_ATTACK), UseMaxHP)*RightArm->GetAPCost());
      }
      if (LeftArm) {
        double Damage = LeftArm->GetDamage();
        if (Damage) Effectivity += 1/(Enemy->GetTimeToDie(this, int(Damage)+1, LeftArm->GetToHitValue(), AttackIsBlockable(GetLeftWielded() ? WEAPON_ATTACK : UNARMED_ATTACK), UseMaxHP)*LeftArm->GetAPCost());
      }
      ++AttackStyles;
    }
  }
  if (IsUsingLegs()) {
    ...
    if (RightLeg || LeftLeg) {
      ...
      if (RightLeg) TimeToDie += Enemy->GetTimeToDie(this, int(RightLeg->GetKickDamage())+1, RightLeg->GetKickToHitValue(), AttackIsBlockable(KICK_ATTACK), UseMaxHP)*RightLeg->GetKickAPCost();
      if (LeftLeg) TimeToDie += Enemy->GetTimeToDie(this, int(LeftLeg->GetKickDamage())+1, LeftLeg->GetKickToHitValue(), AttackIsBlockable(KICK_ATTACK), UseMaxHP)*LeftLeg->GetKickAPCost();
...
int humanoid::GetAttribute (int Identifier, truth AllowBonus) const {
  if (Identifier < BASE_ATTRIBUTES) return character::GetAttribute(Identifier, AllowBonus);
  int Attrib = 0;
...
int humanoid::CheckForBlock (character *Enemy, item *Weapon, double ToHitValue, int Damage, int Success, int Type) {
  ...
  if (GetRightWielded()) Damage = CheckForBlockWithArm(Enemy, Weapon, GetRightArm(), ToHitValue, Damage, Success, Type);
  if (Damage && GetLeftWielded() && (!Weapon || Weapon->Exists())) Damage = CheckForBlockWithArm(Enemy, Weapon, GetLeftArm(), ToHitValue, Damage, Success, Type);
  return Damage;
}
...
truth humanoid::CheckBalance (double KickDamage) {
  return
    ...
    IsStuck() ||
    !KickDamage ||
    (GetUsableLegs() != 1 && !IsFlying() && KickDamage*5 < RAND()%GetSize());
}
...
void humanoid::SWeaponSkillTick () {
  for (std::list<sweaponskill*>::iterator i = SWeaponSkill.begin(); i != SWeaponSkill.end(); ) {
    if ((*i)->Tick() && IsPlayer()) {
      ...
      if (Item) (*i)->AddLevelDownMessage(Item->CHAR_NAME(UNARTICLED));
    }
...
void humanoid::CreateBlockPossibilityVector (blockvector &Vector, double ToHitValue) const {
  ...
  /* Double block */
  if (RightBlockCapability+LeftBlockCapability) Vector.push_back(std::make_pair(RightBlockChance*LeftBlockChance, RightBlockCapability+LeftBlockCapability));
  /* Right block */
  if (RightBlockCapability) Vector.push_back(std::make_pair(RightBlockChance*(1-LeftBlockChance), RightBlockCapability));
  /* Left block */
  if (LeftBlockCapability) Vector.push_back(std::make_pair(LeftBlockChance*(1-RightBlockChance), LeftBlockCapability));
}
...
character *humanoid::CreateZombie () const {
  ...
  for (c = 0; c < Zombie->AllowedWeaponSkillCategories; ++c) Zombie->CWeaponSkill[c] = CWeaponSkill[c];
...
void humanoid::LeprosyHandler () {
  ...
  if (!RAND_N(1000*GetAttribute(ENDURANCE))) DropRandomNonVitalBodypart();
  if (!game::IsInWilderness()) {
    for (int d = 0; d < GetNeighbourSquares(); ++d) {
      lsquare *Square = GetNeighbourLSquare(d);
      if (Square && Square->GetCharacter()) Square->GetCharacter()->TryToInfectWithLeprosy(this);
    }
  }
...
cchar *humanoid::GetRunDescriptionLine (int I) const {
  ...
  if (IsSwimming()) {
    if (!GetRightArm() && !GetLeftArm() && !GetRightLeg() && !GetLeftLeg()) return !I ? "Floating" : "ahead fast";
