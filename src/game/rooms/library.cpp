...
truth library::DropItem (character *Customer, item *ForSale, int Amount) {
  ...
  if (Customer->CanBeSeenBy(GetMaster())) {
    ...
    if (GetMaster()->GetMoney()) {
      ...
      if (Amount == 1)
        ADD_MESSAGE("\"What an interesting %s. I'll pay \1Y%d\2 gold pieces for it.\"", ForSale->CHAR_NAME(UNARTICLED), Price);
      else
        ADD_MESSAGE("\"What an interesting collection of %d %s. I'll pay \1Y%d\2 gold pieces for it.\"", Amount, ForSale->CHAR_NAME(PLURAL), Price);
