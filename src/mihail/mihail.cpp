...
static truth DebugDraw (festring Filename) {
  {
    ...
    if (!HighScore.IsOpen()) return false;
    HighScore.Get();
...
static int OutputHTML (festring InputFilename) {
  highscore Input;
...
int Main (int argc, char **argv) {
  if (argc == 3) {
    ...
    if (Arg == "-l") {
      ...
      if (!Input.GetSize()) std::cout << "No games. No winners.\n";
      std::cout << std::endl;
      for (festring::sizetype c = 0; c < (festring::sizetype)Input.GetSize(); ++c)
        std::cout << c+1 << " " << Input.GetEntry(c).CStr() << " " << Input.GetScore(c) << "\n";
      return 0;
    }
    if (Arg == "-h") return OutputHTML(argv[2]);
    if (Arg == "-d") {
      if (!DebugDraw(argv[2])) {
